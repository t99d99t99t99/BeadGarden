...
![튜토리얼 예시 화면](assets/tutorial/tutorial_7.png)
...
- `index.html`: p5.js 스케치와 전체 스크립트를 불러오는 진입점입니다.
- `sketch.js`: 게임 상태 전환, 캔버스 크기 조정, UI 인스턴스 생성, 입력 이벤트 전달을 담당합니다.
- `style.css`: 기본 페이지 스타일과 캔버스 표시 스타일을 정의합니다.
- `src/`: 게임 로직과 UI 코드가 들어 있는 주요 소스 디렉터리입니다.
- `src/ui/`: 인트로, 튜토리얼, 정원 목록, 화분 설정, 화분 꾸미기, 상세 화면, 줄기 제작 화면 등 화면별 UI 클래스가 들어 있습니다.
- `src/objects/`: 비즈, 화분, 철사처럼 게임 안에서 사용되는 주요 오브젝트 클래스가 들어 있습니다.
- `src/assets/`: 비즈와 화분 스프라이트 아틀라스 로딩 및 매니페스트 코드가 들어 있습니다.
- `src/tools/`: 아틀라스 생성과 Firebase 업로드 같은 보조 도구가 들어 있습니다.
- `src/db.js`: Firebase Firestore와 Storage 연동, 로컬 폴백 데이터 처리를 담당합니다.
- `src/beadGame.js`: 비즈 줄기 만들기 게임의 핵심 진행 로직을 담당합니다.
- `src/handDetector.js`: 웹캠 기반 손 인식과 손 좌표 처리를 담당합니다.
- `src/qrCode.js`: 화분 공유용 QR 코드 생성을 담당합니다.
- `assets/`: 비즈, 화분, 배경, UI 버튼, 튜토리얼 이미지와 영상, 폰트, 사운드 등 런타임 리소스가 들어 있습니다.
- `libraries/`: p5.js, p5.sound, ml5.js, Matter.js 같은 브라우저 라이브러리가 들어 있습니다.
- `local/`: 개발 중 참고용 이미지와 문서가 들어 있는 로컬 작업 자료 디렉터리입니다.

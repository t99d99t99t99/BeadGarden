...
function drawPotComposition(pot, x, y, w, h, options = {}) {
  ...
  const drawStemHighlight = (points, accent, outerWeight, innerWeight) => {
    stroke(accent[0], accent[1], accent[2], accent[3]);
    strokeWeight(innerWeight + 6);
    drawPotRenderPath(points);
  };
  ...
  for (let i = 0; i < stems.length; i++) {
    if (i === options.selectedStemIndex) {
      drawStemHighlight(stems[i].displayPoints, [255, 0, 255, 180], 24, 16);
    } else if (
      i === options.hoveredStemIndex ||
      (
        options.highlightHover &&
        distanceToPotRenderPath(mouseX, mouseY, stems[i].displayPoints) < 15
      )
    ) {
      drawStemHighlight(stems[i].displayPoints, [210, 0, 255, 160], 20, 13);
    }
  }
